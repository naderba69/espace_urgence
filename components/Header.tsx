...
export default function Header() {
  ...
  return (
    <>
      <header style={{ paddingTop: "env(safe-area-inset-top)" }} className="eutn-header no-print relative z-40 shrink-0 border-b border-line/70 bg-surface/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center gap-2 px-3 py-2">
          ...
          <Link href="/" className="flex items-center gap-2.5 font-extrabold text-lg tracking-tight">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-teal-500 to-emerald-600 text-white shadow-md shadow-teal-600/30">
              <Activity className="h-5 w-5" aria-hidden />
            </span>
            <span className="hidden bg-gradient-to-r from-teal-500 to-emerald-400 bg-clip-text text-transparent sm:inline">{t("app.name")}</span>
          </Link>

          <div className="mx-auto hidden min-w-0 max-w-md md:block md:flex-1">
            <SearchBar />
          </div>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label={t("nav.navigation")}>
          ...
          >
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-line px-4 py-3">
              <span className="flex items-center gap-2.5 font-extrabold">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-teal-500 to-emerald-600 text-white shadow-md shadow-teal-600/30">
