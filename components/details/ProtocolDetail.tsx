      ...
      <section aria-label="steps" className="flex flex-col gap-3">
        {protocol.steps.map((s, i) => (
          <div key={i} className="card flex gap-3 rounded-2xl border border-line bg-surface p-4">
            ...
            <div>
              <p className="font-semibold leading-snug">{lang === "ar" ? s.title.ar : s.title.fr}</p>
              {s.detail && <p className="mt-1 text-sm opacity-80">{lang === "ar" ? s.detail.ar : s.detail.fr}</p>}
            </div>
          </div>
        ))}
      </section>
          ...
          <div className="flex flex-col gap-3">
            {protocol.trajectory.map((tr, i) => (
              <div key={i} className="rounded-xl border border-line bg-surface p-3">
                <p className="font-semibold text-amber-500"><T fr={tr.when.fr} ar={tr.when.ar} /></p>
                <ul className="mt-1 list-disc space-y-1 ps-5 text-sm">
                  {tr.do.map((d, j) => (
                    <li key={j}><T fr={d.fr} ar={d.ar} /></li>
                  ))}
                </ul>
              </div>
            ))}
