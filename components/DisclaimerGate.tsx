...
export default function DisclaimerGate() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- lecture localStorage après hydratation (anti-mismatch)
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" role="alertdialog" aria-modal="true" aria-label={t("disclaimer.title")}>
      <div className="w-full max-w-lg rounded-2xl border border-amber-500/50 bg-surface p-6 shadow-2xl">
        ...
        <button
          ...
          className="mt-5 w-full rounded-xl bg-blue-600 px-4 py-3 text-lg font-bold text-white hover:bg-blue-500"
          autoFocus
        >
          {t("disclaimer.accept")}
        </button>
