...
import { readAiConfig, writeAiConfig, subscribeAiConfig, DEFAULT_AI, PROVIDER_PRESETS, clearAiData, type AiProvider } from "@/lib/ai-config";
...
export default function AiSection() {
  const { t, lang } = useApp();
  // Config lue via le mini-store : snapshot stable, MàJ auto à l'écriture,
  // snapshot serveur = DEFAULT_AI (pas de mismatch d'hydratation).
  const cfg = useSyncExternalStore(subscribeAiConfig, readAiConfig, () => DEFAULT_AI);
  ...
  return (
    <section className="card flex flex-col gap-4 rounded-2xl border border-blue-600/40 bg-surface p-4" aria-label="IA">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button role="switch" aria-checked={cfg.enabled} onClick={() => save({ enabled: !cfg.enabled })}
          className={`touch w-16 rounded-full border-2 px-1 py-1 transition ${cfg.enabled ? "border-blue-600 bg-blue-600 justify-end" : "border-line bg-surface2 justify-start"} flex`}>
        ...
        <>
          ...
          <div className="grid gap-2 sm:grid-cols-2" role="group" aria-label="provider">
            {(Object.keys(PROVIDER_PRESETS) as AiProvider[]).map((p) => (
              <button key={p} onClick={() => {
                ...
                className={`touch rounded-xl border px-4 py-3 font-bold ${cfg.provider === p ? "border-blue-600 bg-blue-600 text-white" : "border-line hover:bg-surface2"}`}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="flex flex-col gap-1 text-sm font-semibold">
              ...
              <input value={cfg.textModel} onChange={(e) => save({ textModel: e.target.value.trim() })}
                className="rounded-xl border border-line bg-surface2 px-3 py-2 font-mono text-sm outline-none focus:ring-2 focus:ring-blue-600" />
            </label>
            <label className="flex flex-col gap-1 text-sm font-semibold">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={runTest} disabled={testing || !cfg.apiKey.trim()}
              className="touch gap-2 rounded-xl bg-blue-600 px-5 py-2 font-bold text-white hover:bg-blue-500 disabled:opacity-40">
              {testing ? <Loader2 className="h-5 w-5 animate-spin" aria-hidden /> : null}
              {lang === "ar" ? "اختبر الاتصال" : "Tester la connexion"}
            </button>
            {testResult === "ok" && (
              <span className="flex items-center gap-1 font-bold text-blue-500"><CheckCircle2 className="h-5 w-5" /> OK ✓</span>
            )}
            {testResult === "ko" && (
