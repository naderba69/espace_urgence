...
export default function InstallPwa({ variant }: { variant: "banner" | "icon" | "drawer" }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- détection PWA/localStorage après hydratation (anti-mismatch)
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center" role="alertdialog" aria-modal="true">
      <div className="card w-full max-w-sm rounded-2xl border border-line bg-surface p-5">
        <p className="mb-3 flex items-center gap-2 text-lg font-black">
          <Share className="h-5 w-5 text-blue-500" aria-hidden />
          {isIos
        ...
        <button
          ...
          className="touch mt-4 w-full rounded-xl bg-blue-600 py-2.5 font-bold text-white"
        >
  ...
  if (variant === "icon") {
    ...
    return (
      ...
      >
        <Download className="h-5 w-5 text-blue-500" aria-hidden />
      </button>
    );
  }
  ...
  if (variant === "drawer") {
    ...
    return (
      <>
        <button
          onClick={() => void install()}
          className="touch gap-3 rounded-xl border border-blue-600/50 bg-blue-600/10 px-4 py-3 font-semibold text-blue-500 transition hover:bg-blue-600/20"
        >
  ...
  return (
    <>
      <div className="card relative flex items-center gap-3 rounded-2xl border border-blue-600/60 bg-gradient-to-r from-blue-600/15 to-transparent p-4">
        <Smartphone className="h-8 w-8 shrink-0 text-blue-500" aria-hidden />
        ...
        <button
          onClick={() => void install()}
          className="touch flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 font-black text-white shadow-lg hover:bg-blue-500"
        >
