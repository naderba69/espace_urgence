...
export default function ECGHistory({ refreshKey }: { refreshKey: number }) {
  ...
  // Chargement IndexedDB : setState dans la promesse (pas synchrone dans l'effet),
  // avec garde anti-retour si le composant est démonté entre-temps.
  useEffect(() => {
    let alive = true;
    listEcgRecords().then((r) => { if (alive) setRecords(r); });
    return () => { alive = false; };
  }, [refreshKey]);

  // Rechargement déclenché par les boutons (supprimer / tout effacer)
  const reload = () => { void listEcgRecords().then(setRecords); };
    ...
    <section className="card rounded-2xl border border-line bg-surface p-4">
      ...
      <ul className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {records.map((r) => {
          ...
          return (
            <li key={r.id} className={`card relative overflow-hidden rounded-xl border ${on ? "border-blue-600 ring-2 ring-blue-600" : "border-line"}`}>
              <button onClick={() => r.id !== undefined && toggleSel(r.id)} aria-pressed={on} className="block w-full">
                <div className="relative aspect-[4/3] w-full bg-black">
                  <Image src={r.thumb} alt="ECG" fill className="object-contain" unoptimized />
                </div>
                <div className="p-2 text-start">
                  <p className="line-clamp-2 text-xs font-bold">{r.analysis.suspectedDiagnosis ?? "—"}</p>
