...
const SEV_STYLE = {
  normal: { cls: "border-blue-600 bg-blue-600/10", text: "text-blue-500", Icon: CircleCheck, fr: "Aspect rassurant", ar: "مظهر مطمئن" },
...
export default function ECGResultCard({ analysis, raw }: { analysis: EcgAnalysis; raw: string }) {
  ...
  // v17.2 — liens résolus par le fichier de références (plus de base de protocoles).
  const refMap = useRefIndexMap();
  const protocols = resolveRefLinks(refMap, "protocole", analysis.protocolIds ?? []);
      ...
      {protocols.length > 0 && (
        <div className="mt-3">
          ...
          <ul className="flex flex-wrap gap-2">
            {protocols.map((p) => (
              <li key={p.id}>
                <Link href={p.href} className="touch gap-1 rounded-full bg-blue-600 px-4 py-2 text-sm font-bold text-white">
