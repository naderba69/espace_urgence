...
import { protocolsPhase7 as protocolsP7 } from "./protocols-p7";
import { protocolsPhase8 as protocolsP8 } from "./protocols-p8";
import { protocolsPhase9 as protocolsP9 } from "./protocols-p9";
import { protocolsPhase10 as protocolsP10 } from "./protocols-p10";
import { protocolsPhase11 as protocolsP11 } from "./protocols-p11";
import { protocolsPhase12 as protocolsP12 } from "./protocols-p12";

// v17.2 — الفئات انتقلت إلى `data/protocol-categories.ts` وأُعيد تصديرها هنا للتوافق.
// Les catégories vivent désormais dans `data/protocol-categories.ts` (module léger) ;
// elles restent ré-exportées ici pour ne casser aucun import existant.
export { protocolCategories } from "./protocol-categories";
export type { ProtocolCategory } from "./protocol-categories";
...
const protocolsCore: Protocol[] = [
  ...
  exams: {
    bio: [{ fr: "Gazométrie, K+, Ca2+, Mg2+, glycémie (causes réversibles 5H/5T)", ar: "غازات دم، بوتاسيوم، كالسيوم، مغنازيوم، سكر (أسباب عكوسة)" }],
    img: [{ fr: "Coronarographie si sus-décalage ST post-ROSC ou origine suspectée", ar: "قثطرة قلبية إن ارتفاع ST بعد العودة أو سبب مشتبه" }],
    ecg: [{ fr: "Analyse du rythme q2 min + ECG 12 dérivations post-ROSC", ar: "تحليل الإيقاع كل دقيقتين + تخطيط 12 مشتقة بعد العودة" }],
  },
    steps: [
      { title: { fr: "Constater : absence de réponse, respiration absente ou gasps, pas de pouls carotidien < 10 s", ar: "التأكد: لا استجابة، تنفس غائب أو لهاث، لا نبض سباتي خلال أقل من 10 ث" }, detail: { fr: "Alerter/Composer SAMU 190 — activer la chaîne de survie.", ar: "نادِ/اتصل بالإسعاف 190 — فعّل سلسلة الإنقاذ." } },
      { title: { fr: "RCP haute qualité : 100-120/min, profondeur 5-6 cm, relâchement complet, interruptions < 10 s", ar: "إنعاش جيد: 100-120/د، عمق 5-6 سم، ارتداد كامل، توقفات أقل من 10 ث" } },
      ...
      { title: { fr: "Voie aérienne avancée + capnographie (EtCO2). Cible : EtCO2 > 10-20 mmHg", ar: "مجرى هوائي متقدم + كابنوغرافيا (الهدف EtCO2 أكثر من 10-20)" } },
      { title: { fr: "ROSC : ABCDE, TA ≥ PAM 65, ECG 12 dérivations, capno, T° 32-36 °C si coma, bilan", ar: "استعادة الدوران: ABCDE، متوسط ضغط 65 أو أكثر، تخطيط 12 اشتقاق، حرارة 32-36°م إن غيبوبة، فحوص" } },
    ],
    keyPoints: [
    ...
    trajectory: [
      {
        ...
        do: [
          { fr: "Si PAS < 90 ou SpO2 < 92 % : remplissage prudent + noradrénaline PSE (titré PAM 65).", ar: "إذا ضغط انقباضي أقل من 90 أو تشبع أقل من 92: تعويض حذر + نورأدرينالين معاير (متوسط 65)." },
          { fr: "ECG 12 dérivations immédiat ; si STEMI → régulation 190/QEEG-PCI.", ar: "ECG 12 فوراً؛ إذا STEMI ← تنظيم 190/قسطرة." },
        ],
      },
      {
        when: { fr: "Arrêt réfractaire > 20 min sans cause réversible trouvée", ar: "توقف مقاوم أكثر من 20 د دون سبب عكوس" },
        do: [
          { fr: "Avis médical régulateur 190 pour discussion poursuite/arrêt des soins (facteurs de mauvais pronostic : âge, comorbidités, temps sans RCP, EtCO2 < 10 après 20 min).", ar: "رأي الطبيب المنظم 190 لمناقشة المتابعة/الإيقاف (سوء الإنذار: العمر، الأمراض، مدة بلا إنعاش، EtCO2 أقل من 10 بعد 20 د)." },
  ...
  exams: {
    bio: [{ fr: "Tryptase 1-2 h après début (confirmation) — ne retarde jamais l'adrénaline", ar: "تريبتاز بعد 1-2 س من البدء (تأكيد) — لا يؤخّر الأدرينالين أبداً" }],
    ecg: [{ fr: "ECG si douleur/instabilité (syndrome de Kounis)", ar: "تخطيط إن ألم/لا استقرار (متلازمة كونيس)" }],
  },
    steps: [
      ...
      { title: { fr: "ADRÉNALINE IM immédiate : 1 mg/mL face antéro-latérale de cuisse", ar: "أدرنالين عضلياً فوراً: 1ملغ/مل في الفخذ الأمامي الوحشي" }, detail: { fr: "> 12 ans/adulte : 500 µg (0,5 mL) — 6-12 ans : 300 µg — 6 mois à 6 ans : 150 µg — < 6 mois : 100-150 µg. Répéter après 5 min si non amélioré.", ar: "أكثر من 12 سنة: 500 مكغ (0.5 مل) — 6-12 سنة: 300 — 6 أشهر إلى 6 سنوات: 150 — أقل من 6 أشهر: 100-150. تُعاد بعد 5 دقائق عند عدم التحسن." } },
  ...
  exams: {
    bio: [{ fr: "Glycémie, hémostase, plaquettes, créatinine", ar: "سكر، تخثر، صفيحات، كرياتينين" }],
    img: [{ fr: "Scanner cérébral sans injection immédiat + angioscanner ± perfusion", ar: "سكانر دماغ بلا حقن فوراً + سكانر أوعية ± تروية" }],
    ecg: [{ fr: "ECG : recherche de FA (source emboligène)", ar: "تخطيط: بحث عن رجفان أذيني (مصدر صمي)" }],
  },
    steps: [
      ...
      { title: { fr: "Scanner cérébral SANS injection en urgence (< 25 min idéalement)", ar: "سكانر دماغي بلا حقن مستعجلاً (أقل من 25 د مثالياً)" }, detail: { fr: "Hémorragie → avis neurochirurgical + contrôle TA. Ischémie → voir reperfusion.", ar: "نزف → رأي جراحة الأعصاب + ضبط الضغط. إقفار → انظر إعادة التروية." } },
      { title: { fr: "AVC ischémique < 4,5 h : discuter thrombolyse (altéplase 0,9 mg/kg, max 90 mg, 10 % bolus) selon critères d'inclusion/exclusion", ar: "إقفارية أقل من 4.5 س: ناقش حلّ الخثرة (ألتيبلاز 0.9 ملغ/كغ أقصى 90، 10% دفعة) حسب المعايير" } },
      { title: { fr: "Occlusion gros vaisseau : thrombectomie mécanique < 6 h (jusqu'à 24 h selon imagerie) — orientation centre référent", ar: "انسداد وعاء كبير: استئصال خثرة آلي أقل من 6 س (إلى 24 س حسب التصوير) — توجيه لمركز مرجعي" } },
      { title: { fr: "TA : ne traiter que si > 220/120 (ou > 185/110 si thrombolyse prévue)", ar: "الضغط: عالج فقط إذا أكثر من 220/120 (أو أكثر من 185/110 إذا حلّ خثرة مزمع)" } },
      { title: { fr: "NPO + test de déglutition ; O2 seulement si SpO2 < 94 % ; prévenir aspiration", ar: "صيام + اختبار بلع؛ أكسجين فقط إذا تشبع أقل من 94%؛ وقاية من الاستنشاق" } },
    ],
    keyPoints: [
  ...
  exams: {
    bio: [{ fr: "Hb/Hte itératives, hémostase, gazos/lactates, groupage", ar: "خضاب متكرر، تخثر، غازات/لاكتات، زمرة" }],
    img: [{ fr: "FAST élargi, radio bassin/thorax, scanner corps entier si stabilisé", ar: "FAST موسع، صورة حوض/صدر، سكانر شامل إن مستقر" }],
  },
    steps: [
    ...
    trajectory: [
      ...
      { when: { fr: "Choc persistant malgré 1 L de cristalloïdes", ar: "صدمة مستمرة رغم لتر من المحلول" }, do: [
        { fr: "Hémorragie occulte (thorax/abdomen/bassin/rétro-péritoine) : FAST, ceinture pelvienne ; transfusion massive (CGR:PFC ≈ 1:1) ; acide tranexamique < 3 h.", ar: "نزف خفي (صدر/بطن/حوض): FAST، حزام حوضي؛ نقل دم واسع (كريات:بلازما ≈ 1:1)؛ حمض ترانيكساميك أقل من 3 س." },
      ]},
  ...
  exams: {
    bio: [{ fr: "Glycémie capillaire (D10 5 mL/kg si hypoglycémie)", ar: "سكر شعيري (D10 5 مل/كغ إن نقص)" }, { fr: "K+/Ca2+ si ACR réfractaire ; gaz du sang si prolongé", ar: "بوتاسيوم/كالسيوم إن إنعاش معند؛ غازات دم إن طال" }],
    img: [{ fr: "Radio thorax après intubation (position sonde)", ar: "صورة صدر بعد التنبيب (موضع الأنبوب)" }],
    ecg: [{ fr: "Rythme au scope : FV/TV sans pouls choquable vs asystolie/PEA", ar: "نظم على الشاشة: رجفان/تسرع بطيني قابل للصدم مقابل سكون/نشاط بلا نبض" }],
  },
    steps: [
...
export const protocols: Protocol[] = [...protocolsCore, ...protocolsP2, ...protocolsP3, ...protocolsP4, ...protocolsP5, ...protocolsP6, ...protocolsP7, ...protocolsP8, ...protocolsP9, ...protocolsP10, ...protocolsP11, ...protocolsP12];
