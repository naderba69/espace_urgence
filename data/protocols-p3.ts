...
export const protocolsP3: Protocol[] = [
  {
    ...
    severity: "critical",
    summary: { fr: "Saignement génital après accouchement : massage utérin, ocytocine, TXA < 3 h, chirurgie si échec.", ar: "نزيف تناسلي بعد الولادة: تدليك رحمي، أوكسيتوسين، TXA أقل من 3 س." },
  exams: {
    bio: [{ fr: "Hb, hémostase + fibrinogène (coagulopathie)", ar: "خضاب، تخثر + فيبرينوجين (اعتلال تخثر)" }],
    img: [{ fr: "Écho utérin (rétention) / exploration chirurgicale", ar: "إيكو رحم (احتباس) / استكشاف جراحي" }],
  },
    steps: [
      ...
      { title: { fr: "ACIDE TRANEXAMIQUE 1 g IV sur 10 min < 3 h (répétable ×1 à 30 min)", ar: "حمض ترانيكساميك 1 غ وريد على 10 د خلال أقل من 3س (تُعاد مرة بعد 30 د)" } },
    ...
    trajectory: [
      ...
      { when: { fr: "Choc hémorragique", ar: "صدمة نزفية" }, do: [
        { fr: "2 VVP + NaCl ; acide tranexamique 1 g IV (< 3 h) ; transfusion CGR (4) + PFC ; noradrénaline si remplissage insuffisant.", ar: "خطان + ملح؛ حمض ترانيكساميك 1 غ وريدياً (أقل من 3 س)؛ نقل كريات (4) + بلازما؛ نورأدرينالين إن كان التعويض غير كافٍ." },
      ]},
      { when: { fr: "Coagulopathie (fibrinogène < 2 g/L)", ar: "اضطراب تخثر (فيبرينوجين أقل من 2 غ/ل)" }, do: [
  ...
  exams: {
    bio: [{ fr: "Glycémie capillaire (éliminer hypoglycémie) + SpO2", ar: "سكر شعيري (استبعاد نقص السكر) + تشبع" }, { fr: "Ionogramme + créatinine si agitation/contention prolongée (rhabdomyolyse)", ar: "شاردة + كرياتينين إن هياج/تقييد مطوّل (انحلال ربيدات)" }, { fr: "Dépistage toxique orienté par le contexte", ar: "فحص سموم موجّه بالسياق" }],
    img: [{ fr: "Scanner cérébral seulement si trauma crânien ou signes focaux", ar: "سكانر دماغ فقط إن رضّ رأسي أو علامات بؤرية" }],
    ecg: [{ fr: "ECG : QT avant halopéridol", ar: "تخطيط: QT قبل هالوبيريدول" }],
  },
    steps: [
    ...
    trajectory: [
      ...
      { when: { fr: "Après sédation : FR < 10, SpO₂ < 94 %, hypotension", ar: "بعد التهدئة: تنفس أقل من 10، تشبع أقل من 94%، هبوط ضغط" }, do: [
      ...
      { when: { fr: "Complication : T° ≥ 40 °, rigidité, CPK ↑", ar: "مضاعفة: حرارة 40 أو أكثر، تصلب، ارتفاع CPK" }, do: [
  ...
  exams: {
    bio: [{ fr: "Paracétamolémie à H4 (abaque Rumack-Matthew), ASAT/ALAT, INR, créatinine", ar: "مستوى باراسيتامول س4 (مخطط روماك-ماثيو)، أنزيمات، INR، كرياتينين" }],
  },
    steps: [
      ...
      { title: { fr: "Ingestion unique > 150 mg/kg (ou > 200 quel que soit contexte de risque) : NAC sans attendre le taux", ar: "ابتلاع وحيد أكثر من 150 ملغ/كغ: ابدأ الـNAC دون انتظار التحليل" } },
    ...
    keyPoints: [
      ...
      { fr: "NAC = efficace surtout < 8 h ; bénéfice possible au-delà. Ne jamais stopper pour une réaction anaphylactoïde légère.", ar: "NAC أمثل أقل من 8س وفعّال بعدها أيضاً. لا توقفه لتفاعل خفيف." },
    ],
    trajectory: [
      ...
      { when: { fr: "24-72 h : cytolyse (ASAT ↑), TP < 50 %, encéphalopathie", ar: "24-72 س: تحلل كبدي (ارتفاع ASAT)، TP أقل من 50%، اعتلال دماغي" }, do: [
  ...
  exams: {
    bio: [{ fr: "Cholinestérases si disponibles (diagnostic clinique d'abord)", ar: "كولينستيرات إن توفرت (التشخيص سريري أولاً)" }],
    img: [{ fr: "Radio thorax (hypersécrétion bronchique)", ar: "صورة صدر (فرط إفراز قصبي)" }],
  },
    steps: [
      ...
      { title: { fr: "ATROPINE : 1-2 mg IV bullet, répéter en doublant jusqu'à atropinisation (sécheresse, cœur > 80, pupilles)", ar: "أتروبين: 1-2 ملغ وريد دفعة، ضاعِف حتى الأتروبنة (جفاف، نبضأكثر من 80، حدقة)" }, detail: { fr: "Des doses massives cumulées sont parfois nécessaires (dizaines de mg) — titrer à l'effet, pas à un total.", ar: "قد تلزم جرعات تراكمية ضخمة — عايِر على التأثير لا المجموع." } },
    ...
    keyPoints: [
      { fr: "Carbamates (insecticides agricoles fréquents en Tunisie) : même tableau clinique, durée plus courte (< 6 h), pas de syndrome intermédiaire — l'atropinisation reste le traitement ; la pralidoxime est inutile (voire déconseillée).", ar: "الكاربامات (مبيدات فلاحية شائعة في تونس): نفس الصورة السريرية، مدة أقصر (< 6 س)، دون متلازمة وسيطة — الإشباع بالأتروبين يبقى العلاج؛ البراليدوكسيم بلا فائدة (بل غير منصوح به)." },
    ...
    trajectory: [
      ...
      { when: { fr: "Amélioration", ar: "تحسّن" }, do: [
        { fr: "Surveillance ≥ 72 h (risque de rebond) ; décontamination complète (peau, vêtements) ; consignes d'éviction.", ar: "مراقبة 72 أو أكثر س (خطر الارتداد)؛ تنقية كاملة (جلد، ملابس)؛ تعليمات الإبعاد عن المصدر." },
      ]},
    ],
  ...
  exams: {
    bio: [{ fr: "Hémostase (anticoagulants), glycémie", ar: "تخثر (مميعات)، سكر" }],
    img: [{ fr: "Scanner cérébral selon règles Canadian CT / TRC", ar: "سكانر دماغ حسب قواعد كندية" }],
  },
    steps: [
      ...
      { title: { fr: "GCS ≤ 8 ou risque ventilatoire : IOT précoce (RSI) — NE PAS hyperventiler de routine", ar: "غلاسكو 8 أو أقل أو خطر تنفسي: تنبيب مبكر (RSI) — بلا تهوية مفرطة روتينية" } },
      { title: { fr: "PAS cible ≥ 100 à 110 mmHg (âge-dépendant) ; SpO2 ≥ 94 % ; normoglycémie ; normothermie ; tête surélevée 30°", ar: "انقباضي 100 أو أكثر إلى 110 حسب العمر؛ تشبع 94 أو أكثر%؛ سكر طبيعي؛ حرارة طبيعية؛ رأس مرفوع 30°" } },
  ...
  exams: {
    bio: [{ fr: "Hte, créatinine, CO-Hb si feu en espace clos", ar: "حجم كريات، كرياتينين، CO-Hb إن حريق مكان مغلق" }],
  },
    steps: [
      ...
      { title: { fr: "Remplissage Parkland si > 20 % adulte (> 10-15 % enfant) : 4 mL × kg × % en 24 h, moitié en 8 h", ar: "توسيع باركلاند إن أكثر من 20% كبير (10-15% طفل): 4 مل×كغ×% في 24 س، نصفها بأول 8 س" } },
      ...
      { title: { fr: "Critères de brûlure grave : face/mains/périnée, circulaire, inhalation, > 5 % (enfant) ou > 10 % adulte 2e degré → centre brûlés", ar: "حروق خطيرة: وجه/يدان/عجان، دائرية، استنشاق، أكثر من 5% طفل أو أكثر من 10% كبير درجة ثانية ← مركز حروق" } },
    ],
    keyPoints: [
    ...
    trajectory: [
      ...
      { when: { fr: "Oligurie malgré le remplissage (< 0,5 mL/kg/h)", ar: "قلة بول رغم التعويض (أقل من 0.5 مل/كغ/س)" }, do: [
      ...
      { when: { fr: "Hypothermie < 35 °C", ar: "هبوط حرارة أقل من 35" }, do: [
  ...
  exams: {
    bio: [{ fr: "Glycémie, iono (Na : tonicité), gazos si sévère", ar: "سكر، شوارد (صوديوم: توتر)، غازات إن شديد" }],
  },
    steps: [
    ...
    trajectory: [
      ...
      { when: { fr: "Convulsion sur hyponatrémie", ar: "تشنج على نقص صوديوم" }, do: [
        { fr: "NaCl hypertonique 3 % : 3 mL/kg IV ; hospitalisation ; contrôle natrémie (ne pas corriger > 8-10 mmol/L/24 h).", ar: "ملح مفرط التوتر 3%: 3 مل/كغ وريدياً؛ تنويم؛ رقابة الصوديوم (لا تصحيح أكثر من 8-10 ميلي مكافئ/24 س)." },
      ]},
    ],
    medications: [],
