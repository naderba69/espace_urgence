...
export default function ChecklistsPage() {
  const { lang, t } = useApp();
  useRegisterRecent("outil:checklists");
  const [checks, setChecks] = useState<Checks>({});

  // eslint-disable-next-line react-hooks/set-state-in-effect -- restauration localStorage après hydratation (serveur rendu par défaut, anti-mismatch)
  ...
  return (
    <div className="flex max-w-3xl flex-col gap-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <PageHeader icon={<CheckCircle2 className="h-6 w-6" />} title={t("checklists.title")} sub={t("checklists.intro")} count={CHECKLISTS.length} />
      </header>
      ...
      {CHECKLISTS.map((cl) => {
        ...
        return (
          <section
            key={cl.id}
            id={cl.id}
            className="card overflow-hidden rounded-2xl border border-line bg-surface"
          >
            <div className="flex items-start justify-between gap-3 border-b border-line p-4">
              ...
              <div className="flex shrink-0 gap-1">
                <button
                  ...
                  className="touch rounded-xl border border-line p-2 hover:bg-blue-600/10"
                >
            ...
            <div className="flex items-center gap-3 px-4 pt-3">
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-line" role="progressbar" aria-valuenow={p.pct} aria-valuemin={0} aria-valuemax={100}>
                <div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${p.pct}%` }} />
              </div>
            ...
            <div className="flex flex-col gap-4 p-4">
              {cl.groups.map((g, gi) => (
                <div key={gi}>
                  <h3 className="mb-2 text-sm font-black uppercase tracking-wide text-blue-600">
                        ...
                        <li key={ii}>
                          <button
                            ...
                            className={`touch flex w-full items-start gap-3 rounded-xl border p-3 text-start text-sm leading-snug transition ${
                              on ? "border-blue-600/40 bg-blue-600/10 opacity-60" : "border-line hover:bg-blue-600/5"
                            }`}
                            aria-pressed={on}
                          >
                            {on ? (
            ...
            <footer className="border-t border-line bg-blue-600/5 px-4 py-2 text-xs opacity-70">
