...
import { useRefIndexMap, resolveRefLinks } from "@/lib/ref-index";
...
export default function TriagePage() {
  const { lang } = useApp();
  useRegisterRecent("outil:triage");
  const [system, setSystem] = useState<"" | TriageCase["system"]>("");
  const [open, setOpen] = useState<string | null>(null);
  // v17.2 — fichier de références (14 Ko gzip, précaché) au lieu de la base des protocoles.
    ...
    <div className="flex max-w-3xl flex-col gap-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="flex items-center gap-2 text-2xl font-extrabold">
          <ListFilter className="h-7 w-7 text-blue-500" aria-hidden />
          <T fr="Triage — situation par situation" ar="الفرز — حالة بحالة" />
        </h1>
        <Link
          href="/triage-ia"
          className="touch flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 font-bold text-white hover:bg-blue-500"
      ...
      <div className="flex flex-wrap gap-2">
        <button
          onClick={() => setSystem("")}
          className={`touch rounded-full px-4 py-1.5 text-sm font-bold ${system === "" ? "bg-blue-600 text-white" : "bg-surface2 border border-line"}`}
        >
          <T fr="Tous" ar="الكل" />
        </button>
        {CHIP_ORDER.filter((s) => TRIAGE_CASES.some((c) => c.system === s)).map((s) => (
          <button
            ...
            className={`touch rounded-full px-4 py-1.5 text-sm font-bold ${system === s ? "bg-blue-600 text-white" : "bg-surface2 border border-line"}`}
          >
      ...
      <ul className="flex flex-col gap-3">
        {list.map((c) => {
          ...
          // v17.2 — titres résolus par le fichier de références (14 Ko) : la page ne
          // télécharge plus la base des 98 protocoles pour afficher deux ou trois liens.
          const protos = resolveRefLinks(refMap, "protocole", c.protocolIds);
          return (
            <li key={c.id} className={`card scroll-mt-16 overflow-hidden rounded-2xl border border-line bg-surface ${c.priority === "P1" ? "sev-strip sev-critical" : c.priority === "P2" ? "sev-strip sev-urgent" : "sev-strip sev-standard"}`}>
              <button
                onClick={(e) => { if (!isOpen) reveal((e.currentTarget as HTMLElement).closest("li")); setOpen(isOpen ? null : c.id); }}
              ...
              {isOpen && (
                <div className="border-t border-line p-4 pt-3 text-sm">
                  ...
                  <section className="mb-3">
                    <h3 className="mb-1 flex items-center gap-2 font-bold text-blue-600 dark:text-blue-400">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {protos.map((p) => (
                        <Link key={p.id} href={p.href}
                          className="touch inline-flex items-center gap-1 rounded-lg bg-blue-600/10 px-3 py-1.5 font-bold text-blue-700 hover:bg-blue-600/20 dark:text-blue-300">
