...
const CAT = {
  ...
  vert: { cls: "bg-blue-600", fr: "VERT — peut attendre (réévaluation programmée)", ar: "أخضر — يمكنه الانتظار (إعادة تقييم)" },
} as const;
...
export default function TriagePage() {
  ...
  // v17.2 — les protocoles suggérés sont résolus par le fichier de références.
  const refMap = useRefIndexMap();
  const protocols = resolveRefLinks(refMap, "protocole", res?.protocolIds ?? []);

  return (
    <div className="flex max-w-2xl flex-col gap-5">
      <h1 className="flex items-center gap-2 text-2xl font-extrabold"><ListFilter className="h-7 w-7 text-blue-500" /> {t("ai.triage.title")}</h1>
      <AiGate title={t("ai.triage.title")}>
        <div className="card flex flex-col gap-4 rounded-2xl border border-line bg-surface p-4">
          <label className="flex flex-col gap-1 font-semibold">
            ...
            <textarea value={motif} onChange={(e) => setMotif(e.target.value)} rows={3}
              ...
              className="rounded-xl border border-line bg-surface2 px-3 py-3 outline-none focus:ring-2 focus:ring-blue-600" />
          </label>
            ...
            {(["fc", "fr", "ta", "spo2", "gcs"] as const).map((k) => (
              <label key={k} className="flex flex-col gap-1 text-xs font-bold">
                ...
                <input value={vitals[k]} onChange={(e) => setVitals((v) => ({ ...v, [k]: e.target.value }))} inputMode="decimal"
                  className="rounded-xl border border-line bg-surface2 px-2 py-2 text-center tabular-nums outline-none focus:ring-2 focus:ring-blue-600" />
              </label>
            ))}
          </div>
        ...
        <div ref={resRef} className="scroll-mt-16" aria-hidden />
          ...
          <div className={`rounded-2xl p-5 text-white ${CAT[res.category].cls}`}>
            ...
            {protocols.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {protocols.map((p) => (
                  <Link key={p.id} href={p.href} className="rounded-full bg-black/25 px-4 py-2 text-sm font-bold">
