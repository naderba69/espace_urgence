...
import { Check, Copy, Eraser, Printer, FileText, Download } from "lucide-react";
...
export default function FicheSamuPage() {
  const { lang, t, hydrated } = useApp();
  useRegisterRecent("outil:fiche-samu");
  ...
  // v14.6 — استيراد خط الزمن الدوائي (توثيق ذاتي): أول فتح بحالة فارغة فقط.
  const importTimeline = (force = false) => {
    try {
      const raw = localStorage.getItem("eutn:timeline");
      if (!raw) return 0;
      const evs = JSON.parse(raw) as { f: string; a: string; t: number; m: number }[];
      if (!Array.isArray(evs) || evs.length === 0) return 0;
      const hhmm = (t: number) => {
        const d = new Date(t);
        return `${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
      };
      const lines = [...evs].sort((a, b) => a.t - b.t).map((e) => {
        const nm = lang === "ar" ? e.a : e.f;
        return `${hhmm(e.t)} — ${nm}`;
      });
      setS((p) => (force || !p.traitements.trim() ? { ...p, traitements: lines.join("\n") } : p));
      return lines.length;
    } catch { return 0; }
  };

  useEffect(() => {
    if (!hydrated) return; // بعد ترطيب التفضيلات: أسماء بلغة المستخدم لا الافتراضية
    // eslint-disable-next-line react-hooks/set-state-in-effect -- تعبئة أولية من التخزين المحلي بعد الترطيب (نمط الاستعادة)
    importTimeline();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- الاستيراد الأولي فقط
  }, [hydrated]);
  ...
  const importBtn = () => {
    const occupied = Boolean(s.traitements.trim());
    if (occupied && !window.confirm(L("Le champ Traitements n'est pas vide : l'import remplacera son contenu ?", "حقل «العلاجات» غير فارغ: سيُستبدل محتواه بالاستيراد؟"))) return; // المستخدم رفض الاستبدال
    const n = importTimeline(occupied);
    window.alert(n > 0 ? L(`${n} doses importées depuis la chronologie.`, `استُوردت ${n} إعطاءات من خط الزمن.`) : L("Aucune dose dans la chronologie.", "لا إعطاءات في خط الزمن."));
  };

  const inputCls =
    "w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm outline-none focus:border-blue-600";
  ...
  return (
    <div className="flex max-w-3xl flex-col gap-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <PageHeader icon={<FileText className="h-6 w-6" />} title={t("fiche.title")} sub={t("fiche.intro")} />
      </header>

      {/* Confidentialité */}
      <div className="rounded-2xl border border-blue-600/30 bg-blue-600/10 p-4 text-sm font-semibold">
        {t("fiche.privacy")}
      </div>

      {/* Actions */}
      <div className="flex flex-wrap gap-2">
        <button onClick={copy} className="touch flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white shadow hover:bg-blue-500 active:scale-95">
          {copied ? <Check className="h-4 w-4" aria-hidden /> : <Copy className="h-4 w-4" aria-hidden />}
          {copied ? L("Copié ✓", "تم النسخ ✓") : L("Copier en texte", "نسخ كنص")}
        </button>
        <button onClick={() => window.print()} className="touch flex items-center gap-2 rounded-xl border border-line px-4 py-2.5 text-sm font-bold hover:bg-blue-600/10 active:scale-95">
          <Printer className="h-4 w-4" aria-hidden />{L("Imprimer / PDF", "طباعة / PDF")}
        </button>
        <button onClick={importBtn} className="touch flex items-center gap-2 rounded-xl border border-blue-600/40 px-4 py-2.5 text-sm font-bold text-blue-600 hover:bg-blue-600/10 active:scale-95">
          <Download className="h-4 w-4" aria-hidden />{L("Importer chronologie", "استيراد الخط الزمني")}
        </button>
      ...
      <section className="card rounded-2xl border border-line bg-surface p-4">
        <h2 className="mb-3 font-black text-blue-600">1 · {L("Mission", "التدخل")}</h2>
      ...
      {/* Patient + bilan */}
      <section className="card rounded-2xl border border-line bg-surface p-4">
        <h2 className="mb-3 font-black text-blue-600">2 · {L("Patient & bilan", "المريض والفحص")}</h2>
      ...
      {/* Gestes */}
      <section className="card rounded-2xl border border-line bg-surface p-4">
        <h2 className="mb-3 font-black text-blue-600">3 · {L("Gestes réalisés", "الإجراءات المنجزة")}</h2>
        <div className="flex flex-wrap gap-2">
          {GESTES.map((g) => {
            ...
            return (
              <button
                ...
                className={`touch rounded-full border px-3.5 py-1.5 text-sm font-bold transition ${
                  on ? "border-blue-600 bg-blue-600 text-white" : "border-line hover:bg-blue-600/10"
                }`}
              >
                {lbl}
              </button>
      ...
      {/* Orientation */}
      <section className="card rounded-2xl border border-line bg-surface p-4">
        <h2 className="mb-3 font-black text-blue-600">4 · {L("Orientation & transmissions", "التوجيه والإبلاغ")}</h2>
