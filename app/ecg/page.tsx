  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <PageHeader icon={<HeartPulse className="h-6 w-6" />} title={<T fr="ECG — rythmes essentiels" ar="تخطيط القلب — الإيقاعات الأساسية" />} />
      </header>
        ...
        <div className="mt-3 flex flex-wrap gap-2" role="tablist" aria-label="rythmes">
          {ecgRhythms.map((r) => (
            <button key={r.id} role="tab" aria-selected={rhythm.id === r.id}
              onClick={() => { setRhythm(r); reveal(ficheRef.current); }}
              className={`touch rounded-xl border px-3 py-2 text-sm font-semibold ${rhythm.id === r.id ? "border-blue-600 bg-blue-600 text-white" : "border-line hover:bg-surface2"}`}>
      ...
      <section ref={ficheRef} className="card scroll-mt-16 rounded-2xl border border-line bg-surface p-4">
        <h2 className="mb-3 text-xl font-extrabold text-blue-500"><T fr={rhythm.title.fr} ar={rhythm.title.ar} /></h2>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-xl bg-blue-600/10 p-3 border border-blue-600/40">
            <p className="mb-1 flex items-center gap-2 text-sm font-bold text-blue-500"><ClipboardCheck className="h-4 w-4" aria-hidden /><T fr="Conduite" ar="التصرّف" /></p>
      ...
      {/* Lien analyseur IA */}
      <section className="card flex items-start gap-4 rounded-2xl border border-blue-600/50 bg-blue-600/5 p-4">
        <Camera className="h-8 w-8 shrink-0 text-blue-500" aria-hidden />
        <div className="flex-1">
        ...
        <Link href="/ecg-analyzer" className="touch self-center rounded-xl bg-blue-600 px-4 py-2 font-bold text-white">
          <T fr="Ouvrir" ar="افتح" />
        </Link>
      </section>
    </div>
