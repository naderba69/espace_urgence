  ...
  return (
    <div className="flex max-w-2xl flex-col gap-5">
      ...
      <div className="card flex flex-col gap-4 rounded-2xl border border-line bg-surface p-4">
        ...
        <label className="flex w-full min-w-0 flex-col gap-1 font-semibold">
          ...
          <select value={idx} onChange={(e) => pick(Number(e.target.value))}
            className="w-full min-w-0 rounded-xl border border-line bg-surface2 px-3 py-3 text-base outline-none focus:ring-2 focus:ring-blue-600">
            {PERFUSIONS.map((x, i) => {
        ...
        {p.weightBased && (
          <label className="flex flex-col gap-1 font-semibold">
            ...
            <NumStepper value={ weight } onValue={ setWeight } className="rounded-xl border border-line bg-surface2 px-3 py-3 text-center text-2xl tabular-nums outline-none focus:ring-2 focus:ring-blue-600" label="weight" />
          </label>
        )}

        {/* Tableau des paliers */}
        <div>
          <p className="mb-2 flex items-center justify-between font-semibold text-blue-500">
              ...
              <tbody>
                {(() => {
                  ...
                  return rows.map((d) => {
                    ...
                    return (
                      <tr key={d} className={Math.abs(d - doseClamped) < 1e-9 ? "bg-blue-600 text-white font-bold" : "odd:bg-surface2/50"}>
        ...
        <div className="flex items-end gap-2">
          <label className="flex flex-1 flex-col gap-1 text-sm font-semibold">
            ...
            <NumStepper value={String(dose)} onValue={(v) => setDose(Number(v))} step={p.doseStep} placeholder={String(p.doseStart ?? "")} className="rounded-xl border border-line bg-surface2 px-3 py-3 text-center text-xl tabular-nums outline-none focus:ring-2 focus:ring-blue-600" label="dose" />
          </label>
        ...
        {p.tips && (
          <div className="rounded-xl border border-line bg-surface2 p-3 text-sm">
            <p className="mb-1 flex items-center gap-2 font-bold text-blue-600 dark:text-blue-400">
        ...
        {med && (
          <Link href={`/medicaments/${med.id}`} className="flex items-center gap-1 self-end text-sm font-semibold text-blue-600 hover:underline dark:text-blue-400">
