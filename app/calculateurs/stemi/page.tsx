...
export default function StemiPage() {
  ...
  const bool = (v: boolean, set: (b: boolean) => void, fr: string, ar: string) => (
    <button
      onClick={() => set(!v)}
      className={`touch rounded-xl border-2 px-4 py-2.5 text-start text-sm font-bold ${v ? "border-blue-500 bg-blue-600/15 text-blue-700 dark:text-blue-300" : "border-line bg-surface2"}`}
      aria-pressed={v}
    >
      {lang === "ar" ? ar : fr}
    </button>
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-5">
      ...
      <div className="card flex flex-col gap-2 rounded-2xl border border-line bg-surface p-4">
        <p className="mb-1 font-bold text-blue-600 dark:text-blue-400"><T fr="2. Modificateurs" ar="2. واصفات" /></p>
      ...
      {/* Résultat */}
      {res ? (
        <div className={`rounded-2xl border-2 p-4 ${res.critical ? "border-red-600 bg-red-600/10" : "border-blue-600 bg-blue-600/10"}`}>
          <p className={`flex items-center gap-2 font-black ${res.critical ? "text-red-600 dark:text-red-400" : "text-blue-700 dark:text-blue-300"}`}>
