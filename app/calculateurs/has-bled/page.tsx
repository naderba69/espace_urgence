        ...
        {ITEMS.map((it) => {
          ...
          return (
            <button key={it.id} role="checkbox" aria-checked={isOn}
              onClick={() => { setOn((p) => { const n = new Set(p); if (isOn) { n.delete(it.id); } else { n.add(it.id); } return n; }); trackEvent("calculator_use", { id: "has-bled" }); }}
              className={`touch justify-between gap-3 rounded-xl border px-4 py-3 text-start font-semibold ${isOn ? "border-blue-600 bg-blue-600/15 text-blue-400" : "border-line hover:bg-surface2"}`}>
              <span><T fr={it.label.fr} ar={it.label.ar} /></span>
              <span className="shrink-0 whitespace-nowrap tabular-nums font-black text-blue-500">+1</span>
            </button>
          );
        })}

        <div className={`mt-2 rounded-2xl p-5 text-center text-white ${high ? "bg-red-600" : "bg-blue-600"}`}>
