...
export default function WellsPage() {
  ...
  const interp = likely
    ...
    : { fr: "EP improbable (≤4) — D-dimères: si négatifs, EP exclue (selon contexte)", ar: "صمة غير مرجّحة (≤4) — D-dimer: إن سلبي تُستبعد (حسب السياق)", cls: "bg-blue-600" };
        ...
        {ITEMS.map((i) => {
          ...
          return (
            <button key={i.id} role="checkbox" aria-checked={on}
              onClick={() => { setChecked((p) => { const n = new Set(p); if (on) { n.delete(i.id); } else { n.add(i.id); } return n; }); trackEvent("calculator_use", { id: "wells" }); }}
              className={`touch justify-between gap-3 rounded-xl border px-4 py-3 text-start font-semibold ${on ? "border-blue-600 bg-blue-600/15 text-blue-400" : "border-line hover:bg-surface2"}`}>
