...
const BANDS = [
  { fr: "Pas de déficit mesuré", ar: "لا عجز مقاس", cls: "bg-blue-600" },
  { fr: "Mineur (1–4)", ar: "خفيف (1–4)", cls: "bg-blue-700" },
      ...
      <div className="grid gap-3">
        {ITEMS.map((it) => (
          <fieldset key={it.id} className="card rounded-2xl border border-line bg-surface p-3">
            <legend className="px-1 text-sm font-bold text-blue-500">{it.id}. <T fr={it.title.fr} ar={it.title.ar} /></legend>
              ...
              {it.options.map((o) => {
                ...
                return (
                  <button key={o.v} role="radio" aria-checked={on}
                    ...
                    className={`touch justify-between rounded-lg border px-2.5 py-2 text-start text-sm ${on ? "border-blue-600 bg-blue-600/15 text-blue-400 font-bold" : "border-line hover:bg-surface2"}`}>
