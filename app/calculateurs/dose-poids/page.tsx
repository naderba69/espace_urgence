      ...
      <div className="card rounded-2xl border border-line bg-surface p-4 flex flex-col gap-4">
        <label className="flex flex-col gap-1 font-semibold">
          ...
          <select
            ...
            className="rounded-xl border border-line bg-surface2 px-3 py-3 text-base outline-none focus:ring-2 focus:ring-blue-600"
          >
            {eligible.map((m) => (
        ...
        <label className="flex flex-col gap-1 font-semibold">
          ...
          <input
            ...
            className="rounded-xl border border-line bg-surface2 px-3 py-3 text-2xl text-center tabular-nums outline-none focus:ring-2 focus:ring-blue-600"
          />
        </label>
          ...
          <ul className="divide-y divide-line">
            {weightResusPanel(w).map((row) => (
              <li key={row.key} className="flex items-start justify-between gap-3 py-2 text-sm">
                ...
                <span className="shrink-0 rounded-lg bg-surface2 px-2.5 py-1 font-mono font-black tabular-nums text-blue-500">{row.dose}</span>
              </li>
            ))}
          </ul>
        </section>
