  ...
  return (
    <div className="flex max-w-2xl flex-col gap-5">
      <PageHeader
        ...
        title={<T fr="Adrénaline IM — anaphylaxie" ar="أدرينالين عضلياً — تأق" />}
        sub={<T fr="Dose selon l'âge + grades et timing des répétitions." ar="جرعة حسب العمر + الدرجات وتوقيت التكرار." />}
      />
      <div className="card flex flex-col gap-2 rounded-2xl border border-line bg-surface p-4 text-sm font-bold">
        <p><T fr="Grade 1: peau seule (urticaire) — surveiller." ar="درجة ١: جلد فقط (شرى) — مراقبة." /></p>
        <p><T fr="Grade 2: + respi/abdo → adrénaline IM face latérale cuisse." ar="درجة ٢: + تنفس/بطن → أدرينالين عضلي." /></p>
        <p><T fr="Grade 3-4: choc/arrêt → adrénaline + remplissage; répéter toutes les 5 min." ar="درجة ٣-٤: صدمة/توقف → أدرينالين + تعويض؛ كرر كل ٥ د." /></p>
      </div>

      <div className="card rounded-2xl border border-line bg-surface p-4">
        <p className="mb-3 font-semibold text-blue-500">{lang === "ar" ? "حسب العمر (RCUK/ERC)" : "Selon l'âge (RCUK/ERC)"}</p>
      ...
      <div className="card rounded-2xl border border-line bg-surface p-4">
        <p className="mb-2 font-semibold text-blue-500">{lang === "ar" ? "أو بالوزن (0.01 ملغ/كغ)" : "Ou selon le poids (0,01 mg/kg)"}</p>
        <div className="flex items-center gap-3">
          <input type="number" min="0" step="0.1" inputMode="decimal" value={weight} onChange={(e) => setWeight(e.target.value)}
            placeholder={t("common.weight")}
            className="w-32 rounded-xl border border-line bg-surface2 px-3 py-3 text-xl text-center tabular-nums outline-none focus:ring-2 focus:ring-blue-600" />
          {weightDose !== null && (
            <p className="rounded-xl bg-blue-600 px-4 py-2 font-black text-white tabular-nums">
