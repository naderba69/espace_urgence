...
export default function GcsPage() {
  ...
  const interp =
    ...
    : { fr: "Trouble léger — réévaluer régulièrement", ar: "خفيف — أعد التقييم دورياً", cls: "bg-blue-600" };
    ...
    <div className="flex max-w-2xl flex-col gap-5">
      ...
      <div className="flex gap-2" role="group" aria-label="adulte/pédiatrie">
        <button
          ...
          aria-pressed={!peds}
          className={`touch flex-1 gap-2 rounded-xl border px-4 py-3 font-bold ${!peds ? "border-blue-600 bg-blue-600 text-white" : "border-line bg-surface hover:bg-surface2"}`}
        >
      ...
      {sections.map((sec) => (
        <fieldset key={sec.id + (peds ? "-p" : "-a")} className="card rounded-2xl border border-line bg-surface p-4">
          <legend className="px-2 font-bold text-blue-500">
              ...
              return (
                <button
                  ...
                  className={`touch justify-between rounded-xl border px-4 py-3 text-start font-medium ${
                    checked ? "border-blue-600 bg-blue-600/15 text-blue-400" : "border-line hover:bg-surface2"
                  }`}
                >
                  <span><T fr={o.label.fr} ar={o.label.ar} /></span>
                  <span className="text-2xl font-black tabular-nums">{o.value}</span>
                </button>
              );
            })}
          </div>
