...
import { ibwKg, tidalVolumeRange, weaningReady, sbtPassed, extubationOk } from "@/lib/calc";

export default function VentilateurPage() {
  const { t, lang } = useApp();
  const [sex, setSex] = useState<"m" | "f">("m");
  const [taille, setTaille] = useState("170");
  const [pre, setPre] = useState<boolean[]>(Array(6).fill(false));
  const [sbt, setSbt] = useState<boolean[]>(Array(4).fill(false));
  const [ext, setExt] = useState<boolean[]>(Array(4).fill(false));
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-5">
      ...
      <div className="card flex flex-col gap-4 rounded-2xl border border-line bg-surface p-4">
        <div className="flex gap-2" role="group" aria-label="sexe">
          {(["m", "f"] as const).map((s) => (
            <button key={s} onClick={() => setSex(s)} aria-pressed={sex === s}
              className={`touch flex-1 rounded-xl border px-4 py-3 font-bold ${sex === s ? "border-blue-600 bg-blue-600 text-white" : "border-line hover:bg-surface2"}`}>
        ...
        <label className="flex flex-col gap-1 font-semibold">
          ...
          <input type="number" inputMode="decimal" value={taille} onChange={(e) => setTaille(e.target.value)}
            className="rounded-xl border border-line bg-surface2 px-3 py-3 text-center text-2xl tabular-nums outline-none focus:ring-2 focus:ring-blue-600" />
        </label>
        ...
        {!Number.isNaN(ibw) && (
          <>
              ...
              <div className="space-y-2">
                {([
                  ...
                  <details key={t} className="rounded-xl border border-line bg-surface2 px-3 py-2">
                    <summary className="cursor-pointer font-bold text-blue-500">{lang === "ar" ? ta : t}</summary>
            ...
            {/* v2.5 — Sevrage / فطام */}
            <div className="rounded-2xl border border-line bg-surface p-4 text-sm">
              <h3 className="mb-3 font-extrabold text-sky-500"><T fr="Sevrage de la ventilation (SBT)" ar="فطام التهوية (اختبار التنفس التلقائي)" /></h3>
              {([
                [pre, setPre, <T key="a" fr="Avant le SBT — tous requis" ar="قبل الاختبار — كلها مطلوبة" />, [
                  ["Cause de la ventilation résolue ou en amélioration", "السبب المُوجِب للتنبيب تحسّن أو زال"],
                  ["SpO2 ≥ 90 % avec FiO2 ≤ 0,5 et PEEP ≤ 8", "تشبع ≥ 90% على FiO2 ≤ 0.5 وPEEP ≤ 8"],
                  ["Hémodynamique stable, sans escalade de vasopresseurs", "استقرار دوري دون تصعيد الموسعات الوعائية"],
                  ["Éveillé, coopératif ou facilement arousable", "صاحٍ متعاون أو سهل الإيقاظ"],
                  ["Toux efficace, sécrétions gérables", "سعال فعال وإفرازات قابلة للإدارة"],
                  ["RSBI < 105 resp/min/L (si mesuré)", "مؤشر RSBI أقل من 105 إن قيس"],
                ]],
                [sbt, setSbt, <T key="b" fr="Pendant/après SBT 30-120 min (CPAP 5 ou pièce en T)" ar="أثناء/بعد الاختبار 30-120 د (CPAP 5 أو قطعة T)" />, [
                  ["FR ≤ 35/min sans détresse respiratoire", "تردد ≤ 35/د دون ضائقة تنفسية"],
                  ["SpO2 ≥ 90 % sans majoration de l'oxygène", "تشبع ≥ 90% دون زيادة الأكسجين"],
                  ["Pas d'instabilité hémodynamique ni trouble de conscience", "لا عدم استقرار دوري ولا اضطراب وعي"],
                  ["Pas de sueurs ni signes d'épuisement", "لا تعرق ولا علامات إنهاك"],
                ]],
                [ext, setExt, <T key="c" fr="Extubation — tous requis" ar="نزع الأنبوب — كلها مطلوبة" />, [
                  ["Fuite au ballonnet présente (cuff-leak test)", "اختبار تسرب الكفة موجود"],
                  ["Sécrétions peu abondantes", "إفرازات قليلة"],
                  ["Niveau de conscience suffisant", "مستوى وعي كافٍ"],
                  ["Toux efficace", "سعال فعال"],
                ]],
              ] as const).map(([vals, setter, head, items], gi) => (
                <fieldset key={gi} className="mb-3">
                  <legend className="mb-1 font-black opacity-90">{head}</legend>
                  <div className="space-y-1">
                    {items.map(([fr, ar], i) => (
                      <label key={fr} className="flex cursor-pointer items-center gap-2 rounded-xl border border-line bg-surface2 px-3 py-2">
                        <input type="checkbox" checked={vals[i]}
                          onChange={() => setter(vals.map((v, j) => (j === i ? !v : v)) as boolean[])}
              ...
              <p className={`rounded-xl p-3 font-black ${weaningReady(pre) && sbtPassed([...sbt]) && extubationOk([...ext]) ? "bg-green-600/15 text-green-500" : weaningReady(pre) && sbtPassed([...sbt]) ? "bg-blue-600/15 text-blue-500" : weaningReady(pre) ? "bg-sky-600/15 text-sky-500" : "bg-amber-500/10 text-amber-500"}`}>
                {weaningReady(pre) && sbtPassed([...sbt]) && extubationOk([...ext])
                  ? <T fr="Extubation possible — surveillance standard post-extubation." ar="يمكن نزع الأنبوب — مراقبة قياسية بعده." />
                  : weaningReady(pre) && sbtPassed([...sbt])
                  ? <T fr="SBT réussi — vérifiez les critères d'extubation." ar="نجح الاختبار — تحقق من معايير نزع الأنبوب." />
                  : weaningReady(pre)
                  ? <T fr="Lancez un SBT de 30 à 120 min (CPAP 5 ou pièce en T)." ar="ابدأ اختبار تنفس تلقائي 30-120 دقيقة (CPAP 5 أو قطعة T)." />
                  : <T fr="Pas prêt pour le SBT — réévaluer quotidiennement." ar="غير جاهز للاختبار — أعد التقييم يومياً." />}
              </p>
            </div>
      ...
      <Link href="/calculateurs/gazometrie"
        className="card flex items-center justify-center gap-2 rounded-2xl border border-blue-600/40 bg-blue-600/10 p-4 font-black text-blue-500 hover:bg-blue-600/20">
        <Wind className="h-5 w-5" aria-hidden />
        <T fr="Ajuster selon la gazométrie — interprétation + tables" ar="عدّل وفق الغازات: تفسير وجداول" />
      </Link>
