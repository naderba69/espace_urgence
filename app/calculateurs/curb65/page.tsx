...
const CONDUITES = [
  { max: 1, fr: "0–1: faible gravité — traitement ambulatoire possible (réévaluer)", ar: "0–1: شدة منخفضة — علاج خارجي ممكن (أعد التقييم)", cls: "bg-blue-600" },
        ...
        {CRITERIA.map((c) => {
          ...
          return (
            <button key={c.id} role="checkbox" aria-checked={on}
              onClick={() => { setChecked((p) => { const n = new Set(p); if (on) { n.delete(c.id); } else { n.add(c.id); } return n; }); trackEvent("calculator_use", { id: "curb65" }); }}
              className={`touch justify-start gap-3 rounded-xl border px-4 py-3 text-start font-semibold ${on ? "border-blue-600 bg-blue-600/15 text-blue-400" : "border-line hover:bg-surface2"}`}>
              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg font-black ${on ? "bg-blue-600 text-white" : "bg-surface2"}`}>{c.letter}</span>
